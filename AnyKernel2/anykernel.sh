...
dump_boot() {
  ...
  ui_print "Dump complete";
  $bin/unpackbootimg -i /tmp/anykernel/boot.img -o $split_img;
  ui_print "Image unpack complete";
  if [ $? != 0 ]; then
...
write_boot() {
  cd $split_img;
  cmdline=`cat *-cmdline`;
  cmdline="$cmdline androidboot.selinux=permissive no_console_suspend=1";
  board=`cat *-board`;
  base=`cat *-base`;
  ...
  $bin/mkbootimg --kernel $kernel --ramdisk /tmp/anykernel/ramdisk-new.cpio.gz $second --cmdline "$cmdline" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff  --tags_offset $tagsoff $dtb --output /tmp/anykernel/boot-new.img;
  ui_print "Image pack complete";
